...
export const getTokenInfo = async (addr: string) => {
    ...
    while (!response.ok && Date.now() - timestamp < 3000) {
    ...
    if (!response.ok) {
        console.log(`Cannot get token information: ${addr}`);
...
export const getWalletTokenAccount = async (wallet: PublicKey, isToken2022: boolean = true) => {
    ...
    const walletTokenAccounts = await afx.web3Conn.getTokenAccountsByOwner(wallet, {
